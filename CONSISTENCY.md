...
> 网页版：`docs/`（本目录，GitHub Pages，**冻结于 v1.3.8**）　小程序版：`WeChatProjects/miniprogram-1`（**v1.4.7**）
...
**v1.4.3–v1.4.7 状态**：五轮均为**小程序端独有**的迭代 ——
...
换边按钮不可用时灰显而非消失、新增退出房间（§18）—— **纯 UI 重排 + 为后续皮肤/声效预留入口**，
不触碰实时协议，因此对网页版零影响。
...
| 版本号展示 | 页面右下角灰色小字 `v1.3.8`（`APP_VERSION`，**已冻结**） | 同左（`version` 字段绑定），小程序端当前 **v1.4.7** |
...
## 十八、对局页控件重排：设置区上移 / 两列按钮 / 换边灰显 / 退出房间（v1.4.7，**小程序端独有**）
...
| ④ | 换边按钮不可用后变灰，而不是消失 | `wx:if` → `disabled` + `.btn-off` |
...
### 18.3 ⚠️ 「不可用」应当可见：换边按钮从"消失"改为"变灰"

原写法：

```xml
<button wx:if="{{!isSpectator && canSwap}}" ...>{{swapText}}</button>
```

`canSwap` 为假时按钮**直接消失**。这一改的信息损失有两个：
1. 用户**不知道有这个功能**（换边是本项目 v1.4.0 才上线的重要能力）；
2. 即使知道，也**分不清"功能没了"和"现在不能用"** —— 而这两者对用户的意义完全不同。
...
<button
  wx:if="{{!isSpectator}}"
  class="btn btn-info {{canSwap ? '' : 'btn-off'}}"
  disabled="{{!canSwap}}"
  bindtap="requestSwap"
>{{swapText}}</button>
```

> 保留 `wx:if="{{!isSpectator}}"` 是有意的：**观战者本就没有换边这个能力**，
> 给他一个灰按钮会误导（"我要怎样才能点亮它？"）。而棋手是**暂时不可用**。
> 区分标准：**"能力不存在"才隐藏，"能力存在但条件不满足"要灰显。**

`disabled` 的 `button` 在小程序里不触发 `bindtap`，正好挡掉无效点击 ——
不需要额外写守卫（`requestSwap` 内部的前置检查仍在，作为双保险）。
...
- 主探针 `probe-index-page.js`：**369/369**（新增 AD 段 23 项）
- 反向验证 `verify-v147-reverse.js`：**51/51**（8 组注入）
- 其余 11 个回归脚本全绿
...
| G | 换边按钮退回 `wx:if` 消失 | AD7 / AD7b / AD8 | AD10 / AD1 |
...
- ⚠️⚠️ **"不可用"与"不存在"必须区分对待**（v1.4.7，见 §18.3）：
  能力**不存在** → 隐藏（观战者不该看到换边按钮，那会误导他去找"点亮方法"）；
  能力**存在但条件不满足** → **灰显**（棋手当下不能换边，但要知道这功能在）。
  一律用 `wx:if` 藏起来的代价是**信息损失**：用户分不清"没了"和"用不了"。
  注意 `disabled` 的 `button` 在小程序里**不触发 `bindtap`**，正好省掉一层守卫。
