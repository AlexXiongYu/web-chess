...
> 网页版：`docs/`（本目录，GitHub Pages，**冻结于 v1.3.8**）　小程序版：`WeChatProjects/miniprogram-1`（**v1.7.3**）
...
**v1.5.3–v1.7.3 状态**：语音/素材/皮肤十五轮，全部**小程序端独有**、**零协议改动**。
...
**v1.7.0 把观测搬上屏幕**（§24.9）—— 连续四版落空后确认病根是"诊断只打 console、
真机看不见"＝零观测。本轮**一处疑似病根都没改**，只做观测：
版本角标开诊断面板 + 四态音频自检 + 朝向实测图。
**v1.7.1 修面板入口 + 方向交还用户**（§24.10）：入口点不出来不是手势问题，
而是 `.version-tag{pointer-events:none}` 让 touch 永远到不了元素（**典型假绿**：
探针只查了 wxml 的 `catchtap`，没查 wxss）；方向改为**用户自己拨定**
（选完立即生效 + 落盘），不再由代码推理。
**v1.7.2 修「真机语音全哑」的唯一真因**（§24.11）——
`wx.loadSubpackage` 是**小游戏** API（小程序里恒 `undefined`），
且**主包不能使用分包资源**（微信官方社区定论）⇒ 主包 index 页播分包语音**必然全 404**；
模拟器不校验包边界故"能响"，真机严格校验故"全哑"。
→ 语音**搬回主包**（`app.json` 删 `subPackages`，路径不变）+ 主包瘦身三处
（棋子 PNG 调色板量化 −474KB、spike 源图移出包外 −199KB、语音重编码 −756KB）。
顺带揪出**第二个真缺陷**：音频回调用普通函数导致 `this` 指向 InnerAudioContext
⇒ `this._diagPush` 抛异常，把 `onCanplay` 里的 `start()` 顶掉（见 §24.11 ⑤）。
因全部改的是渲染、样式与本地音频，**对网页版与房间协议零影响**。
**v1.7.3 修「方向**还是**反」的两个真 bug ＋ 语音「不太对」定案为非缺陷**（§24.12）——
用户复测原文：「有声音了，但是不太对，不知道是音频素材本身错了还是代码问题。穗龙动画**还是**反的。」
· 方向：两个真 bug **都不在"方向算法"本身** ——
  ① **横向走完全无视用户选择**（v1.7.1 起）：`_spikePatch` 横向分支硬编码
     `_spikeFlip = (pos.col < p0.col)`，**从未读过 `_spikeLR`**；面板 `diagSetH` 更彻底，
     只记 `_dirH` 供报告文本用、**不写偏好、不落盘、不生效**。
     ⇒ 用户在左右走上的任何选择都到不了代码里。纵向走的是另一条路且正确，
     所以只有"一半方向"失灵 —— 这正是"我说了反了、你说改了、可它还是反的"的由来。
  ② **镜像轴错位导致每次翻面"侧跳"**：`transform-origin` 原为 **69%**
     （早期在**被污染剪影**上量的，非角色真实质心）；实测走路帧 alpha 质心
     = 53.0/52.4/52.6%（均值 **52.7%**）⇒ 每次 `scaleX(-1)` 横向平移 **32.6% 元素宽**
     （≈80px ≈ 半个身位）。**"方向反"与"翻面跳"肉眼同属一类感觉**。
  ③ 连带：朝向实测图取自已移出包外的 `walk.png`（**过期参照**）→ 改从内联 base64 裁帧。
  ④ 新增「↺ 清除方向记忆」按钮：v1.7.1 偏好存在 storage，若在"两方向都反"时按过会**跨版本粘住**
     （与 v1.6.9「被持久化的静音」同类病）。
  ⚠️ **方向判定法则一字未动** —— 素材朝向经 **6 路独立信号**交叉验证（眼白质心 / 头部质量质心 /
  吻部凸出 / 头冠峰值 / 尾部与红色背包质量 / 眼白相对躯体质心）= **18:0 判定"面朝右"**。
· 语音「不太对」= **素材覆盖不均，不是 bug**：重编码经复验**完全无损**
  （时长一致、24kHz 单声道、响度差 ±1.3dB）；真因是 `VOICE_VARIANTS` **槽位空洞**：
  白方 6 角色里 `move` 仅 **2/6**、`check` **0/6**；黑方 8 角色里 `be-checked` **0/8**、
  `move` **6/8**；唯一全覆盖槽位是黑方 `capture`。⇒ 白方任何普通走子听起来都是紫悦/柔柔、
  白方将军退回普通句、黑方被将无声。**88 条里 23 条未剪出 = 素材缺口。**
  已在 `_voiceForMove` 上方落下**完整覆盖矩阵 ＋"用户会听到什么"**，
  并由 `probe-voice-coverage-doc.js` 守住「注释 ＝ 代码 ＝ 磁盘」三者一致。
  > ⚠️ 该探针**首次运行就抓出我手写矩阵里的两处事实错误**（把黑方角色数写成 6、误称黑方 move 全覆盖）
  > —— 这正是「注释当证据必须配机器」的理由。
> ⚠️⚠️ **本轮最值钱的方法论**：**"素材错"与"代码错"必须先用观测分开，再动手。**
> 方向走的是"先证素材无罪 → 再查偏好链路"，5 分钟定位；
> 而 v1.6.6–v1.6.9 在零观测下改代码，改了五轮全无效果（§24.9）。
...
| 版本号展示 | 页面右下角灰色小字 `v1.3.8`（`APP_VERSION`，**已冻结**） | 同左（`version` 字段绑定），小程序端当前 **v1.7.3** |
...
### 24.9 v1.7.0：连续 5 轮"修完没效果" —— 病根是**没有观测能力**

> **这是本项目到目前为止最重要的一条教训，优先级高于本文件任何具体修复。**

**现象**：v1.6.6 / v1.6.7 / v1.6.8 / v1.6.9 连续四个版本，用户真机复测都是
「**还是一点效果都没有**」—— 而且是**两个互不相关的 bug 同时零变化**
（穗龙朝向 + 满局语音）。四个版本、十几处改动、每次都附了"已修复"的推理链，全落空。

**归因**：两个独立 bug 同时完全无变化，概率上指向一个**共同前提**，而不是两处代码。
共同前提就是：**我看不见真机**。

- v1.6.6–v1.6.9 全部诊断都打在 `console.log`；
- 体验版真机**默认没有 console**（要 vConsole 才有），用户不可能去开；
- 于是"加了可观测性"这件事在真机上等于**零**，我实际是在**纯黑箱上做四轮推演**，
  每轮都用"逻辑上应该对了"代替"设备上确实变了"。

**决定性反证**（本轮依次排除，全部推翻）：
| 假设 | 反证 |
| --- | --- |
| 分包里的音频播不了 | `git show c86b299^:app.json` 显示 **v1.6.1 就已经是 voice 分包**，且当时 4 段将杀语音实测能播 |
| 分包声明页缺失导致整个分包加载失败 | `voice/pages/blank/` 四件套齐全 |
| 素材画稿被换成了有问题那版 | 738×256（验收版）与 738×276（当前版）并排目视比对，实质相同、同为面朝右 |
| 改错了文件 / 没上传 | 版本角标读 `wx.getAccountInfoSync()` 平台真实版本号，改错或漏传会显示旧号 |
| 镜像标志被中途重置 | 只有 `executeRestart` / `executeSwap` 两处复位，均为重开/换边语义，非覆盖源 |

> ⚠️ **准则（本文件的最高优先级条款）**：
> **连续两轮"修完没效果"时，第三轮不许再改"疑似病根"，必须先建立观测。**
> 在没有观测能力的情况下继续改代码，产出的是"看起来很努力的推理"，
> 而它的期望收益是零 —— 因为你连"改之前是什么样"都无法知道。

**本轮做法（不改任何疑似病根，只装眼睛）**：

1. **页内诊断面板** `data.diagShow/diagLines/diagImg`
   —— 右下角版本角标**连点 5 次**开启（`catchtap`，不能是 `bindtap`，否则会被父容器吞掉）。
   纯白底 + 黑字 + 等宽字体，手机上拍屏即清晰可读；`z-index: 400` 高于所有弹窗。
2. **`diagSelfTest()`**：逐条试播 4 个真实槽位，结果分**四态**显示：
   - `✓ onPlay` —— 设备**真的出声**，这是唯一铁证；
   - `△ onCanplay` —— 资源取到了但没出声（**必须与 ✓ 区分**，否则会把静默失败当成功）；
   - `✗ onError` —— 带 `errMsg` 原文（404 / 解码失败 / 子包未加载）；
   - `✗ 2.5s 超时` —— 无任何回调，也是一种失败，必须显式列出。
3. **静默路径上屏**：`_voiceDiag` 与 onPlay/onCanplay/onError 全部同步 `_diagPush`。
   （此前这四条静默路径"全都不出声、全都不报错"，只能靠猜。）
4. **走动判决结果上屏**：每步打印 `走 右 (6,3→6,4) → 脸朝左`
   —— 让用户拍屏即可核对「代码认为的朝向」与「眼睛看到的」是否一致。
5. **朝向实测图** `images/_diag/spike_facing.png`（由 `.ci-secrets/_make_facing_png.py` 生成）：
   ①=素材原图（代码当"向右走"用）②=镜像后（当"向左走"用），
   直接请用户指认"哪一行脸朝左"。
   > **准则**：方向类争议**让用户的眼睛做判据**，不要用像素包围盒做间接推断
   > —— 我此前正是用包围盒"确认"了素材朝向，那套推断在 v1.6.9 得出过错误信心。

**探针**：`.ci-secrets/probe-diag-panel.js`（21 项）守住"观测能力本身"。
> 为什么值得单独守：面板被后来的改动弄坏时**不会报错**，
> 只是永远拿不到真机数据 —— 比功能 bug 更隐蔽，且会让排查重新堕入黑箱。
> 反证：对 v1.6.9 源码跑该探针 → **13 FAIL**。

**上传回执（v1.7.0）**：`__APP__` 1667538B ≈ 1628KB、`/voice/` 1774622B ≈ 1733KB、
`__FULL__` 3442160B。git commit `8422922`。
既有探针：spike-follow 38/0、version-badge 5/0、index-page 422/424（Y8 / AE1 为存量失败）。

> ⚠️ 另一条本轮暴露的思维习惯（已在源码头注释记录）：
> 我曾把「v1.6.1 实测能播」当作音频通路完好的证据，但
> `git ls-tree c86b299^` 显示 v1.6.1 **只有 4 个将杀语音**，
> 从未验证过全量 69 个文件。**引用一个未验证的前提**是本轮之前所有失败推断的共同形状。

### 24.10 v1.7.1：入口被 CSS 吃掉 + 方向必须交还给用户

> **本节的第 ② 条比第 ① 条重要得多。它是本项目的核心教训之一。**

用户对 v1.7.0 的反馈原文：

> 「**无法呼出面板，点击五次没反应**……**我已经说了反了，总共就两个方向，
> 正确答案只有唯一的一个了，居然改这么多轮改不成功。**」

**① 入口点不出来 —— 纯粹的我的 bug。**

v1.7.0 给版本角标加了 `catchtap="onVersionTap"`，探针也断言了它存在。
**但 `.version-tag` 的样式里写着 `pointer-events: none`**
（v1.7.0 之前为了"不拦截棋盘点击"而设），
→ `touch` 事件**永远到不了这个元素**，`catchtap` 写了也无效。点几次都没用。

> ⚠️⚠️ **这是本项目最危险的一类 bug：探针全绿、代码"看起来对"、功能是坏的。**
> 根因是**断言只查了 wxml**（`catchtap` 在不在），没查 wxss（点击能不能到达）。
> v1.7.0 的 `probe-diag-panel.js` 21/21 全绿，而入口根本点不动 —— **典型的假绿**。
> **准则**：涉及"能不能点/能不能看到"的断言，必须**同时查 wxml 与 wxss**；
> `pointer-events`、`display`、`z-index`、`opacity:0` 都会让"元素存在"变成"用户摸不到"。

修：`pointer-events: auto` + 热区放大到 88rpx（≈44px，手指可点），
并**废除"连点 5 次"这种隐藏手势** —— 用户此刻需要的是"一眼能找到的入口"，
藏起来等于没有。现在**点 1 次即开**，角标文字直接写「点我」。

**② 方向改不出来 —— 因为我一直要求用户理解我的实现。**

用户的原话点得很准：**总共就两个方向，正确答案只有唯一的一个。**
一个二值方向、改到第 6 版还没对，说明**问题不在代码，在"判据的传递方式"**：

- 我无法在真机上看动画；
- 用户无法把观感翻译成我的实现术语 ——
  而我在前几轮反复使用「**倒放**」「**镜像**」「**起步帧**」这些词，
  甚至把"看动画判断方向"当成用户的任务。
- **这是把实现细节转嫁给使用者。** 他需要回答的只有一件事：**脸朝哪边。**

**修法：把判断权交还给用户，而不是把解释写得更详细。**

| 做法 | 说明 |
| --- | --- |
| 面板直接设问 | 「上下走的时候，穗龙的脸应该朝哪边？」+ 两个大按钮（脸朝左 / 脸朝右） |
| **选完立刻生效** | `_spikeUD` 当场写入实例并 `setStorageSync('chessSpikeUpDown')` → 下一次上下走即体现，**不必等我再改代码、再上传、再等一版** |
| 落盘记住 | 下次启动仍是他选对的那个方向；不会因为我又改代码而退回 |
| 左右走也可校 | 万一错的其实在横向（我此前只盯上下） |
| 结论机器可读 | 「生成结论并复制」→ 剪贴板 `@@CHESS-DIR@@ / UP-DOWN: LEFT / @@END@@`，粘贴即可，无需组织语言 |

**③ 逻辑简化 —— 这才是"方向的真答案"。**

删掉 v1.6.8/v1.6.9 的「**保持上一次朝向**」策略。那个策略让上下走的朝向
**取决于上一步是怎么走的** → 朝向时左时右、不可预期、无法描述
（用户「我真的无法理解」完全合理，因为那个行为本身就不自洽）。

现在：`_spikeFlip = (this._spikeUD === 'left')` —— **上下走用常量**，
同一次对局里始终一致、可预期、可复现。**方向从此不再有"推理链"。**

> ⚠️ **准则（与 §24.9 并列的最高优先级）**：
> **当用户说"我无法理解"时，正确动作是把判断权交回给他（做成开关 / 二选一），
> 而不是把解释写得更详细。** 解释得越细，越证明你在用实现细节替代他的观感。
> 一个二值问题问 6 轮还问不出来，错的一定不是答案，是**提问方式**。

**探针（v1.7.1）**：`probe-diag-panel.js` 30/0，对 v1.7.0 反证 **8 FAIL**，
其中新增「版本角标 `pointer-events` 不为 none」——**专门守住上面那条假绿**。
`repro-spike-follow.js` 39/0，`[B2]` 段重写为"开关驱动"语义：
**必须显式设 `_spikeUD` 再断言**，不得依赖默认值
（默认值一改，旧断言会因"恰好相等"而假绿 —— 这正是本轮的毛病）。

**上传回执（v1.7.1）**：`__APP__` 1670319B ≈ 1631KB、`/voice/` 1774622B、`__FULL__` 3444941B。
git commit `2a6e928`。
...
### 24.11 v1.7.2：语音"真机全哑"的**唯一真因** —— 主包读不到分包资源

> **本节是本项目最有价值的一条平台知识。它不是我的实现错误，是我对微信平台的误解。**

**症状**：开发者工具里**能响**，真机体验版**一声不出**（连续 5 轮修不掉）。

用户真机自检原文：

> ```
> @@CHESS-VOICE@@ 版本 1.7.1 soundEnabled=true 子包=idle loadSubpackage=无
> — 逐条试播 —
> white/K-celestia/move.m4a ✗ 报错 INNERERRCODE:-1100, ERRMSG:在此服务器上找不到所请求的URL。
> ...（4 条音频全部 404）@@END@@
> ```

**两个关键信号**：`loadSubpackage=无`（该 API **不存在**）+ **4 条音频全 404**。

#### ① `wx.loadSubpackage` 是**小游戏** API，小程序里恒为 `undefined`

官方文档挂在 `/minigame/` 路径下 —— **它根本不属于小程序基础库**。
小程序的子包由框架在「跳转到该子包页面」时**自动下载**，官方原文：

> 「**分包预下载目前只支持通过配置方式使用，暂不支持通过调用 API 完成。**」

所以 v1.5.10 写的 `if (wx.loadSubpackage) { wx.loadSubpackage({name:'voice'}) }`
在小程序里**条件恒假** → **子包从未被下载过**。（且它有 `if` 守卫，不报错，静默无效。）

#### ② 更要命：**主包不能使用分包资源**

微信官方社区定论（逐字）：

> 「**主包不能使用分包资源，分包也不能使用其它分包资源。**」

而台词语音恰恰是**主包**的 `pages/index` 在播 ——
于是**每一句都必然 404**，与路径拼得对不对、文件在不在都无关。

> ✅ **这就是「开发者工具能响、真机全哑」的完整解释**：
> **开发者工具模拟器不校验包边界**，主包引用分包文件照样能读到 → 能响；
> 真机严格校验 → 全部 404 → 全哑。
> 凡是"模拟器对、真机错"且**涉及资源加载**的，第一嫌疑就是**包边界**。

#### ③ 证据链（如何确认这不是猜）

| 证据 | 内容 |
| --- | --- |
| git 历史 | `0511e1c`「v1.5.10: 台词语音改为子包托管（主包瘦身）」**引入**子包 |
| 版本对照 | v1.5.9 及以前语音在**主包**内，真机可播（v1.6.1 的 4 段将杀语音实测有声） |
| 真机回执 | `loadSubpackage=无` + 全部 404（见上方原文） |
| 官方文档 | `loadSubpackage` 属小游戏；分包预下载不支持 API |

**④ 处置 —— 语音搬回主包。**

- `app.json` **整块删除 `subPackages`**；路径 `/voice/...` **一字未改**。
- 随之而来的是**主包必须瘦到 2MB 以内**（原本加回语音会变 2.9MB+）。
  三处瘦身，**分辨率与内容零损失**：

| 手段 | 省下 | 说明 |
| --- | --- | --- |
| 棋子 PNG **调色板量化**（256 色、无抖动） | **474KB** | 已是 256×256，量化不降分辨率；扁平动漫风无抖动最干净 |
| spike `walk/idle` **移出包外** | 199KB | 仅用于生成 WXSS 内联 base64，运行时不需要 |
| 音频 **ffmpeg 重编码** | 756KB | 正片 32kbps/24kHz、`_alt` 16kbps/24kHz；65 个文件**全部保留** |

- 主包实测：**原始 2099KB → 编译后 1873165B（≈1830KB）**，余量 ~175KB。

> ⚠️ **准则（体积口径）**：**2MB 限制作用于「上传编译后」的包，不是源码目录的字节数。**
> 上传时 JS/WXSS 会被 minify（本项目 `project.config.json` 里
> `minified/minifyWXSS/minifyWXML` 全 `true`），源码注释与缩进**不进包**；
> 而**图片/音频是二进制、压缩比 ≈ 1.0**。
> 所以"源码目录 2099KB > 2048KB"**并不代表上传失败** —— 文本部分编译后省了约 142KB。
> 本项目校准比 **0.826**（v1.7.1：1974→1631；v1.7.2：估 1873 → 实测 1873.2，两次独立命中）。
> **唯一真值以上传回执的 `subPackageInfo[__FULL__].size` 为准。**

#### ⑤ 本轮**顺带揪出的第二个真 bug**：音频回调里的 `this` 不是 page

修完包边界后跑探针，全部语音断言仍然 `已播=0`。日志里是：

> ```
> [voice-skip 1] 设 src 抛异常 :: this._diagPush is not a function
> ```

根因：`ctx.onPlay(function () { ... this._diagPush(...) })` ——
**普通函数的 `this` 指向 InnerAudioContext 实例**，它没有 `_diagPush`；
而 `_diagPush` 是 v1.7.0 加诊断面板时才塞进回调的，于是：

- 回调一进来就抛异常；
- `onCanplay` 里 `start()` 写在 `this._diagPush(...)` **之后** →
  **异常把 `start()` 顶掉** → 只能靠 300/800ms 兜底定时器补播；
- 屏幕诊断里**永远看不到「onPlay 开播」这一行** ——
  而它正是 v1.7.0 定义的那个"设备真的出声"的**唯一铁证**。

→ 处置：`_playVoiceNow` 内四个回调**全部改箭头函数**（`this` 词法继承 page），
且 `onCanplay` 里 **`start()` 提到 `_diagPush()` 之前**（诊断永远不许挡播放）。

> ⚠️⚠️ **准则（最高优先级，与 §24.9/§24.10 并列）**：
> **往已有回调里加"日志/上屏"这类副作用时，副作用必须排在业务动作之后。**
> 日志是**观测**，播放是**功能**；观测抛异常把功能顶掉，是**本末倒置**。
> 更一般：**任何"为了排查而加的东西"，都不允许成为新的故障源。**

> ⚠️ **准则（回调 this）**：微信 API 的回调（`onXxx`）**不保证 this 指向你的组件/页面**。
> 只要回调里要访问 `this.xxx`，**一律用箭头函数**，不要用 `function`。
> 本项目已因此产生一个"探针全绿时也看不出"的功能缺陷。

#### ⑥ 探针改动（同步本次架构变更）

- `repro-mate-voice.js`：**46/0**。
  删掉全部 `loadSubpackage`/`subPackages` 注册断言，
  改为断言「`onLoad` 不再调用 `loadSubpackage`」「`app.json` 无 `subPackages`」
  「`_ensureVoicePack` **立即同步回调**」「`voice/` 文件在主包内真实存在」。
  体积断言改为**双口径**：raw 仅提示，**硬断言看按 0.826 校准的编译后估值**。
  另把 `lastCtx.fireEnded()` 包成 `fireEnded()` 安全包装 ——
  原先 `lastCtx` 为 `null` 时探针会以 `TypeError` **崩掉**，
  把后面所有节次的结论一起吞掉（v1.6.9 就踩过）。
- `probe-diag-panel.js`：**34/0**（原 30/0，新增 4 条 `[B2]` 段铁律）：
  ① 已定位 `_playVoiceNow` 函数体；② 内无 `ctx.onXxx(function(){})` 写法；
  ③ 四个回调均为箭头函数；④ `onCanplay` 内 `start()` 先于 `_diagPush()`。
  > 这 4 条就是上面那个真 bug 的**守门断言** —— 有它们，同样的写法不会再溜过去。
  > 注意 `[B]` 段原正则写死 `function`，把"改成箭头函数"当成 FAIL；
  > 已改为同时接受两种写法，并在扫描前**剥离注释行**
  > （修复说明的注释里会引用反例写法，不剥离就会把自己写的说明当成违规代码）。

```
probe-diag-panel.js            34 / 0
repro-mate-voice.js            46 / 0
repro-spike-follow.js          40 / 0
repro-version-badge.js          5 / 0
probe-promotion.js             14 / 0
repro-empty-room-swap-restart  11 / 0
probe-index-page.js           422 / 2   （Y8、AE1 为既有失败，与语音/朝向无关）
```

**上传回执（v1.7.2）**：`__FULL__` 1918165B（≈1873.2KB）——
语音搬回主包后**仍**在 2MB 内。git commit 见 `KB_CHANGELOG.md` / `git log`。

**关于方向**：v1.7.1 的「上下走左镜像」开关（`_spikeUD`，默认 `left`）
**未做任何改动** —— 用户明确说过「左右走都是正确的……只要把上下走的时候左右镜像一下」，
现有实现正是如此。本轮**不猜方向、不动方向**，等用户实测反馈。

---

### 24.12 v1.7.3：方向「**还是**反」＝两个真 bug（都不在算法本身）＋ 语音「不太对」定案为**素材缺口**

用户真机复测 v1.7.2 原文：

> 「**有声音了，但是不太对，不知道是音频素材本身错了还是代码问题。穗龙动画还是反的。**」

注意这句话里其实夹着**一个正确诊断请求**：用户自己已经意识到"素材错"与"代码错"是两回事，
只是无法判断。本轮的工作就是**用观测把这两类分开**，再各自处置。

#### ① 「还是反」—— 为什么上一轮改了却像没改

v1.6.8 / v1.6.9 两轮修的都是**上下走**（`_spikeUD` 那条路），而**横向走是另一条代码路径**，
从 v1.7.1 引入 `_spikeLR` 偏好起就**一次也没被接进去**。用户测到的很可能正是横向那半。

**真 bug A：横向走完全无视用户选择（v1.7.1 起）**

```js
// 修前 —— _spikePatch 横向分支
this._spikeFlip = (pos.col < p0.col)     // ← 硬编码"向左走就镜像"，从不读 _spikeLR
// 面板 diagSetH 更彻底：只记 this._dirH 供报告文本用，
//                       不写 _spikeLR、不落盘、不生效 → 用户的选择到不了代码里
```

后果：**纵向（上下走）走的是另一条路且是对的，所以只有"一半方向"失灵**。
这正是"我说了反了 → 你说改了 → 可它还是反的"的完整解释（他复测的很可能是横向）。
→ 修：横向分支接入 `_spikeLR` 偏好并**落盘生效**，与纵向**对称**。

**真 bug B：镜像轴错位 → 每次翻面"侧跳"**

`transform-origin` 原为 **69%** —— 那是早期在一张**被污染剪影**（含背景残留）上量的，
不等于角色真实质心。实测走路帧 alpha 质心：
...
用 69% 当轴 ⇒ 每次 `scaleX(-1)` 角色横向平移 **32.6% 元素宽**（≈80px ≈ 半个身位）。
**"方向反"与"翻面跳"在肉眼上是同一类感觉**，都会被读成"动画不对"。
→ 修：轴改 **53%**（残余偏移 <1%）。

**连带修复 C：朝向实测图是过期参照。**
面板上那张"参照脸"取自 `images/pieces/MLP/spike/walk.png`，
而该文件已在 v1.7.2 瘦身时**移出包外** → 照着它核对**必然对不上**。
→ 改为直接从 `.spike-sprite` 的**内联 base64**（＝真机真正渲染的那张）裁帧生成。

**连带修复 D：新增「↺ 清除方向记忆」按钮。**
v1.7.1 的方向偏好在 storage 里，若用户当时在"两个方向都显示反了"的状态下按过，
该选择会**跨版本粘住**，新版本也救不回来 —— 与 v1.6.9「被持久化的静音」**同类病**。
→ 面板加按钮一次清掉 `SPIKE_UD_KEY` + `SPIKE_LR_KEY`，回到默认。
（新偏好键名 `chessSpikeLeftRight`，取值 `'follow'` / `'reverse'`，默认 `'follow'`。）

> ⚠️ **方向判定法则一字未动。** 素材朝向经 **6 路独立信号**交叉验证：
> 眼白质心 / 头部质量质心 / 吻部凸出 / 头冠峰值 / 尾部与红色背包质量 / 眼白相对躯体质心
> = **18 : 0 判定"面朝右"**，代码里的假设是对的（v1.6.9 已确认）。
> 帧序 1-2-3 为正序；素材血缘（旧版 / v24 / 归一化 / v3 / 内联）方向一致。
> 故本轮**只修"偏好未接入 + 镜像轴偏移 + 参照图过期"，不碰判定法则**。
> 判反会恶化成"所有水平走都反"。

#### ② 「声音不太对」—— **不是 bug，是素材覆盖不均**

用户无从判断，只能听出"不对劲"。先用观测把两类分开：

- **编码无问题**：重编码经复验**完全无损** —— 时长一致、24kHz 单声道、响度差 **±1.3dB**。
  → 排除编码。
- **真因是 `VOICE_VARIANTS` 的槽位空洞**（完整矩阵已写入 `_voiceForMove` 上方）：

| 阵营 | 角色数 | `move` | `check` | `be-captured` | `be-checked` |
| --- | --- | --- | --- | --- | --- |
| **white** | **6** | **2/6**（紫悦、柔柔） | **0/6** | 3 | — |
| **black** | **8** | **6/8**（缺夜月、暗影） | — | — | **0/8** |

唯一**全覆盖**的槽位是黑方 `capture`（8/8）。`p`（兵）在两阵营 `VOICE_SLUGS` 里都为空
→ 永远走 `pickAny` 兜底。
...
⇒ 这是**素材缺口**（88 条台词里 23 条未剪出），**不是代码缺陷**。
代码侧已"有素材必播、无素材退让并轮换"，无进一步优化空间。

**守门**：新增 `.ci-secrets/probe-voice-coverage-doc.js`（**25/0**），
用块提取 + `new Function` 求值解析 `VOICE_SLUGS`/`VOICE_VARIANTS`/`VOICE_SLOT`，
与磁盘 `.m4a` 实际文件**交叉核对**，并校验注释矩阵里的每条断言
（white move=2 / white check=0 / black be-checked=0 / black move=6/8 /
black capture=8/8 / black=8 角色 / white=6 角色）。
> ⚠️ **该探针首次运行 21/1，抓出的是我自己手写矩阵里的两处事实错误**
> （把黑方角色数写成 6、误称黑方 move 全覆盖）。
> 矩阵里的角色数已改为**由源码计算**而非硬编码 —— 这样它不会再骗人。
> **注释当证据，必须配机器。**

#### ③ ⚠️⚠️ 本轮最值钱的方法论

> **"素材错"与"代码错"必须先用观测分开，再动手。**
>
> 同一个"不对"，两类病根混在一起，只能靠观测分离：
> - 方向走的是"**先证素材无罪**（6 路信号 18:0）→ **再查偏好链路**"→ **5 分钟**定位到 bug A；
> - 而 v1.6.6–v1.6.9 在**零观测**下改代码，改了**五轮全无效果**（§24.9 已记录该教训）。
>
> 推论：**用户报"还是不对"时，第一动作不是改代码，是先把现象拆成可判定的子问题。**

**探针（v1.7.3 全绿）**

```
probe-voice-coverage-doc.js     25 / 0   （新增，守"注释=代码=磁盘"）
probe-diag-panel.js             34 / 0
repro-mate-voice.js             46 / 0
repro-spike-facing.js           25 / 0
repro-spike-follow.js           40 / 0
probe-promotion.js              14 / 0
```
> `probe-index-page.js` 422/2（Y8 joinRoom myColor 脚手架、AE1 黑方换边时序）
> —— **均为存量失败，与本次语音/朝向改动无关**，非阻塞。

**上传回执（v1.7.3）**：`__FULL__` **1928203B**（≈1882.8KB / 2048KB，**92%**），
zip buffer 1788739B。方向与语音改动均为纯前端资源与渲染，**对网页版与房间协议零影响**。

> ⚠️ **发版事故与恢复（本轮附带，值得单列）**：
> 提交前用 `git stash` 做探针基线，进程被 **SIGTERM** 打断 ⇒
> `.git/refs/`、packfile `.pack` 与大量对象被移入**回收站**（只剩 `.idx`/`.rev`），
> 仓库一度不可读。**恢复路径**（工作树完整、索引完好）：
> ① 从回收站 `$R*` 找回 `.pack`，按 sha 与存活 `.idx` 比对确认**同一份**；
> ② 从 `$I*` 元数据解析原路径与删除时间；
> ③ 由 reflog 恢复 `refs/heads/master`；
> ④ 用 `git hash-object -w <file>` 从**工作树**重注入 14 个缺失 blob；
> ⑤ `git write-tree` 得到 `80a84effc7dd9cfb00ce8d9d9ac940721cc5b27e`
>    = **与 v1.7.2 的 tree SHA 逐字节相同** ⇒ 历史完整复原。
> 教训：**重建树时 Windows `git ls-files` 转义非 ASCII 路径、Python stdout 用 cp936**，
> 二者都会让中文文件名错位/SHA 不匹配 —— 必须
> `git -c core.quotepath=false` + `PYTHONIOENCODING=utf-8`。详见技能《陷阱清单 · GIT-1》。
